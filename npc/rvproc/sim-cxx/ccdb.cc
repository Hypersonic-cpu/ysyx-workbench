...
#if DBGENA
void
GuestTracer::init_disasm() {
  auto nemu_path = std::string(std::getenv("NEMU_HOME"));
  void* dl_handle;
  dl_handle =
    dlopen((nemu_path + std::string("/tools/capstone/repo/libcapstone.so.5"))
             .c_str(),
           RTLD_LAZY);
  assert(dl_handle);
...
void
GuestTracer::disassemble(char* str, int size, uint64_t pc, uint8_t* code,
                         int nbyte) {
  cs_insn* insn;
...
void
GuestTracer::init_elfsym(const std::string& elf_file) {
  ...
  for (unsigned i = 0; i < sym_count; ++i) {
    ...
    if (type == STT_FUNC) {
      v_warn(elfSyms.find(addr) == elfSyms.end(),
             ...
             " and ", elfSyms.find(addr)->second.name);
...
void
GuestTracer::frame_trace(uint32_t snpc, uint32_t dst, bool is_ret) {
...
void
GuestTracer::inst_trace(uint32_t pc, uint32_t inst) {
  instCnt++;
