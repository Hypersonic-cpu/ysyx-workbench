...
ccdb::inst_trace() {
  ...
  if (is_jalr || is_jal) {
    ...
    uint32_t immI = comm::sext(comm::bits(inst, 31, 20), 12);
    uint32_t immJ = comm::sext(
      (comm::bits(inst, 31, 31) << 20) | (comm::bits(inst, 19, 12) << 12) |
        (comm::bits(inst, 20, 20) << 11) | (comm::bits(inst, 30, 21) << 1),
      21);
  ...
  bool is_csr =
    comm::bits(inst, 6, 2) == 0b11100 && comm::bits(inst, 14, 12) != 0b000;
...
ccdb::frame_trace(uint32_t snpc, uint32_t dst, bool is_ret) {
  ...
  if (is_ret && it != elf_syms.end()) { // NOTE: TCO
    ...
    frame_stk.emplace_front(depth, it->second.name, it->second.addr, ra,
                            read_args());
    if (runtime_dump_opt.frame_stk)
    ...
    frame_stk.emplace_front(depth, it->second.name, it->second.addr, snpc,
                            read_args());
    if (runtime_dump_opt.frame_stk)
      frame_stk.front().printent(std::cerr, "+", true);
