...
module AXIConnBox (
  ...
  always_ff @(posedge clock) begin : Everyting
    $display("++ DEVICE DISP ID = %d AR_FIRE %d %d ++", device_id, io_master_arvalid, io_master_arready);
    $strobe("++ DEVICE STRB ID = %d AR_FIRE %d %d ++", device_id, io_master_arvalid, io_master_arready);
