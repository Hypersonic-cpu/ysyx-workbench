...
#define src1R() do { \
  *src1 = R(rs1); \
  IFDEF(CONFIG_NPSIM_TRACE, s->nptrace.src_reg[0] = rs1); \
} while (0)
#define src2R() do { \
  *src2 = R(rs2); \
  IFDEF(CONFIG_NPSIM_TRACE, s->nptrace.src_reg[1] = rs2); \
} while (0)
...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *rd     = BITS(i, 11, 7);
  ...
  switch (type) {
    case TYPE_R: src1R(); src2R();         dstR(); break;
    case TYPE_I: src1R();          immI(); dstR(); break;
    case TYPE_S: src1R(); src2R(); immS();         break;
    case TYPE_B: src1R(); src2R(); immB();         break;
    case TYPE_U:                   immU(); dstR(); break;
    case TYPE_J:                   immJ(); dstR(); break;
    case TYPE_N: break;
...
static int decode_exec(Decode *s) {
  ...
  IFDEF(CONFIG_NPSIM_TRACE, s->nptrace.pc = s->pc);
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11",
          jalr   , I,
          R(rd) = s->snpc;
          brAbs((src1 + imm) & ((word_t)(-2)));
          IFDEF(CONFIG_FTRACE_ENABLE, frame_trace(s->dnpc, rd, s->snpc));
          );
  INSTPAT("??????? ????? ????? 000 ????? 11000 11",
          beq    , B, brRel(src1 == src2));
  INSTPAT("??????? ????? ????? 001 ????? 11000 11",
          bne    , B, brRel(src1 != src2));
  INSTPAT("??????? ????? ????? 100 ????? 11000 11",
          blt    , B, brRel((sword_t)src1 < (sword_t)src2));
  INSTPAT("??????? ????? ????? 101 ????? 11000 11",
          bge    , B, brRel((sword_t)src1 >= (sword_t)src2));
  INSTPAT("??????? ????? ????? 110 ????? 11000 11",
          bltu   , B, brRel(src1 < src2));
  INSTPAT("??????? ????? ????? 111 ????? 11000 11",
          bgeu   , B, brRel(src1 >= src2));

  INSTPAT("??????? ????? ????? 000 ????? 00000 11",
          lb     , I, Mrlog(R(rd), src1 + imm, 1); R(rd) = SEXT(R(rd), 8));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11",
          lh     , I, Mrlog(R(rd), src1 + imm, 2); R(rd) = SEXT(R(rd), 16));
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11",
          ...
          brAbs(isa_raise_intr(11, s->pc));
          );
  INSTPAT("0011000 00010 00000 000 00000 11100 11",
          mret   , N,
          brAbs(csr(RISCV_CSR_MEPC))
          );
  INSTPAT("??????? ????? ????? ??? ????? ????? ??",
          inv    , N, INV(s->pc));
  INSTPAT_END();
